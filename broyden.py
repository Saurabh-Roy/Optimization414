...
def armijo_goldstein(f,grad_f,a,b,p,x):
    ...
    while not(float(f(*(x + alpha * p))) < float(f(*x) + a * alpha * np.dot(grad_f(*x),p))):
...
def broyden(B, x_init, a, b,f, grad_f):
    xk = x_init
...
broyden(B, x_init,0.45,0.95,f,gradient_f)
